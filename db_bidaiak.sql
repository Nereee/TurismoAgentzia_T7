...
kodBidaiaMota varchar(2) not null primary key,
 ...
 foreign key (IDAgentzia) references agentzia(IDAgentzia) on delete cascade,
...
foreign key (IDBidaia) references bidaia(IDBidaia) on delete cascade
);
...
deskribapena varchar(50),
prezioa decimal,
foreign key (IDBesteZerbitzuak) references zerbitzua(IDZerbitzua) on delete cascade
);
...
foreign key (IDOstatua) references zerbitzua(IDZerbitzua) on delete cascade,
...
foreign key (IDHegaldia) references zerbitzua(IDZerbitzua) on delete cascade
);
...
KodAireIrteera varchar(5) not null,
KodAireHelmuga varchar(5) not null,
foreign key (IDJoanEtorri) references zerbitzua(IDZerbitzua) on delete cascade,
foreign key (KodAirelinea) references airelinea(KodAirelinea),
foreign key (KodAireIrteera) references aireportua(KodAireportua),
foreign key (KodAireHelmuga) references aireportua (KodAireportua)
);
